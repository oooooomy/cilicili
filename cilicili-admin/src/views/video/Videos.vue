<template>
  <div>
    ...
    <a-modal v-model="posterVisible" title="封面预览" @ok="posterVisible = false">
      <img class="poster" :src="base + '/upload-service/file/image/' + openVideo.poster" alt=""/>
    </a-modal>
    <a-modal v-model="videoVisible" title="视频预览" @ok="videoVisible = false">
      <video width="300" height="240" controls>
        <source :src="base + '/upload-service/file/mp4/' + openVideo.url" type="video/mp4">
      </video>
    </a-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    load() {
      this.loading = true
      FindVideoByStatus('true').then((res) => {
        if (res.status) {
          setTimeout(() => {
            this.data = res.data
            this.loading = false
    ...
    confirm(record) {
      console.log(record);
      SetVideoStatus(record.id, 'false').then((res) => {
        if (res.status) {
          this.load()
          this.$message.success("视频撤销审核成功")
        } else {
