  ...
  <div class="login-box">
    <div>
      ...
      <div style="margin-bottom: 20px">
        ...
        <a-button type="link" style="margin-left: 158px"> 忘记密码 ? </a-button>
      </div>
      <a-button
...
<script>
import { AdminLogin, AdminLoginByCode, GetEmailCode } from "../api/admin";
    ...
    sendEmail() {
      if (this.checkEmail()) {
        ...
        GetEmailCode(this.form.email).then((res) => {
          if (res.code === 200) this.$message.success("验证码发送成功");
      ...
      if (this.checkEmail()) {
        if (this.submitType === "1") {
          AdminLogin(this.form).then((res) => {
            ...
            if (res.code === 200) {
              this.$store.commit("user/saveToken", res.data.id);
          ...
          AdminLoginByCode(this.form.email, this.form.code).then((res) => {
            if (res.code === 200) {
              this.$store.commit("user/saveToken", res.data.id);
              this.$store.commit("user/saveLoginUser", res.data);
              setTimeout(() => {
